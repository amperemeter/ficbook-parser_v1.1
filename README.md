...
Если вы хотите отслеживать появление работ с определенными параметрами, вы можете использовать [третьий вид парсера](filter/index-filter.js). Инструкция по его использованию та же, что и для парсера из папки [mongodb](mongodb). Для просмотра инструкции перейдите по [данной ссылке](mongodb/README-MONGODB.md). Не забудьте заменить исполняемый файл, то есть вместо `index-mongodb.js` вы должны скачать `index-filter.js`.
